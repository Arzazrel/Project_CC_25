 ...
 *
 */
public class NGramCountPairsInMap
{
    /**
    ...
    public static class NGramCountMapper extends Mapper<LongWritable, Text, Text, IntWritable>
    {
        ...
        // to take the window size from configuration and to initialize the data structures used for in mapping methodology
        ...
        private void flush(Context context) throws IOException, InterruptedException {
            ...
            for (Map.Entry<String, Integer> entry : wordCountMap.entrySet()) {
                nGram.set(entry.getKey());                  // set the word
                countWritable.set(entry.getValue());        // set the occurrence
                context.write(nGram, countWritable);        // emit
            }
    ...
    public static void main(final String[] args) throws Exception {
        ...
        String jobName = "NGramCountPairsInMap";     // name for the job
        ...
        if (args.length >= 5)           // check if the user entered the seventh argument
        {
            try {
        ...
        while (successfulRuns < numRunsRequested)
        {
            ...
            final Job job = Job.getInstance(conf, jobName + "_run_" + successfulRuns + "_w" + window + "_red_" + numReducer);
            job.setJarByClass(NGramCountPairsInMap.class);
