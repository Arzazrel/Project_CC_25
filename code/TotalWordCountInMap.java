...
/**
 * Class to implement a MapReduce total word count in the inputfiles, in-mapper combining version.
 */
public class TotalWordCountInMap
{
    /**
     * Mapper class for implement the map logic for the total word count
     */
    public static class TotalWordCountMapper extends Mapper<Object, Text, Text, LongWritable>
    {
        private long localCount;                                    // local word counter
        private static final Text word = new Text("word");          // unique key for all the words

        // to initialize the data structures used fo in mapping
        protected void setup(Context context) throws IOException, InterruptedException {
            localCount = 0;  // inizializza il contatore
        }
        ...
        public void map(final Object key, final Text value, final Context context)
            ...
            while (itr.hasMoreTokens()) {
                itr.nextToken();
                if (localCount < Long.MAX_VALUE)    // control check for overflow
                {
                    localCount++;       // update the counter
                } else {
                    flush(context);     // risk overflow, emit partial data
                    localCount++;       // update the counter
                }
            }
        }
        ...
        private void flush(Context context) throws IOException, InterruptedException {
            if (localCount > 0)     // control check
            {
                context.write(word, new LongWritable(localCount));  // emit
                localCount = 0;                                     // reset counter
            }
        }
     ...
     */
    public static class TotalWordCountReducer extends Reducer<Text, LongWritable, Text, LongWritable> {
        private final LongWritable result = new LongWritable();   // var to set the value to associate with the found words

        // reduce function
        public void reduce(final Text key, final Iterable<LongWritable> values, final Context context)
                throws IOException, InterruptedException {
            long sum = 0;                           // var to the sum of the values associated to keys
    ...
    public static void main(final String[] args) throws Exception {
        ...
        String jobName = "TotalWordCountInMap";       // name for the job
        // check the number of argument passed
        if (args.length < 2) {
            System.err.println("Usage: TotalWordCountInMap <input path> <output base path> [numRuns]");
            System.exit(-1);
        }
        ...
        while (successfulRuns < numRunsRequested)
        {
            ...
            job.setJarByClass(TotalWordCountInMap.class);

            job.setOutputKeyClass(Text.class);                  // set the typer for the output key for reducer
            job.setOutputValueClass(LongWritable.class);        // set the typer for the output value for reducer
